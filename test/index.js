...
describe('RpcClient', function () {
  ...
  it('should handle connection rejected 401 unauthorized', function (done) {
    ...
    client.getBalance('n28S35tqEMbt6vNad7A5K3mZ7vdn8dZ86X', 6, function (
      error,
      parsedBuf
    ) {
      requestStub.restore()
      ...
      error.message.should.equal(
        'Trivechain JSON-RPC: Connection Rejected: 401 Unnauthorized'
      )
      done()
    })
  })
  ...
  it('should handle connection rejected 401 forbidden', function (done) {
    ...
    client.getDifficulty(function (error, parsedBuf) {
      ...
      should.exist(error)
      error.message.should.equal(
        'Trivechain JSON-RPC: Connection Rejected: 403 Forbidden'
      )
      done()
    })
  })
  ...
  it('should handle 500 work limit exceeded error', function (done) {
    ...
    client.getDifficulty(function (error, parsedBuf) {
      ...
      should.exist(error)
      error.message.should.equal(
        'Trivechain JSON-RPC: Work queue depth exceeded'
      )
      done()
    })
  })
  ...
  it('should handle EPIPE error case 1', function (done) {
    ...
    client.getDifficulty(function (error, parsedBuf) {
      ...
      should.exist(error)
      error.message.should.equal(
        'Trivechain JSON-RPC: Request Error: write EPIPE'
      )
      done()
    })
  })
  ...
  it('should handle ECONNREFUSED error', function (done) {
    ...
    client.getDifficulty(function (error, parsedBuf) {
      ...
      should.exist(error)
      error.message.should.equal(
        'Trivechain JSON-RPC: Request Error: connect ECONNREFUSED'
      )
      done()
    })
  })
  ...
  it('should callback with error if invalid json', function (done) {
    ...
    client.getDifficulty(function (error, parsedBuf) {
      ...
      should.exist(error)
      error.message.should.equal(
        'Trivechain JSON-RPC: Error Parsing JSON: Unexpected token o in JSON at position 1'
      )
      done()
    })
  })
  ...
  it('should callback with error if blank response', function (done) {
    ...
    client.getDifficulty(function (error, parsedBuf) {
      ...
      should.exist(error)
      error.message.should.equal(
        'Trivechain JSON-RPC: Error Parsing JSON: Unexpected end of JSON input'
      )
      done()
    })
  })
